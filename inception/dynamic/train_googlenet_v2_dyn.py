...
epochs = 4
niter = (400 * epochs) + 1  # One ResNet epoch is equal to 3990 steps because
...
solver.net.forward()
batches[0] = 0
training_loss[0] = solver.net.blobs['loss3/loss'].data
print batches[0], ',', training_loss[0]
j = 0
for i in range(1, niter):
    solver.step(n_steps)
    training_loss[i] = solver.net.blobs['loss3/loss'].data
    if i == n:
        EMA = np.average(training_loss[0:n-1])
    if i > n:
        EMAprev = EMA
        EMA = ema(n, training_loss[i], EMAprev)
